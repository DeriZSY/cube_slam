...
{
	ros::NodeHandle n;
	ros::Publisher pub_slam_all_poses = n.advertise<geometry_msgs::PoseArray>("/slam_pose_array", 10); // slam pose
...
void incremental_build_graph(Eigen::MatrixXd &offline_pred_frame_objects, Eigen::MatrixXd &init_frame_poses, Eigen::MatrixXd &truth_frame_poses)
{
	...
	for (int frame_index = 0; frame_index < total_frame_number; frame_index++)
	{
		...
		{
			//start detect cuboid
			cv::Mat raw_rgb_img = cv::imread(base_folder + "raw_imgs/" + frame_index_c + "_rgb_raw.jpg", 1); // read a gray-scale image
			...
			raw_2d_objs.leftCols<2>().array() -= 1;
